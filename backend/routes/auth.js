...
router.post('/sync', async (req, res) => {
  ...
  const existing = await db.prepare('SELECT * FROM users WHERE firebase_uid = ?').get(decoded.uid);
  if (existing) return res.json({ user: toProfile(existing) });

  const { fullName, handle } = req.body;
  if (!fullName || !handle) {
  ...
  const handleTaken = await db.prepare('SELECT id FROM users WHERE handle = ?').get(handle.toLowerCase());
  if (handleTaken) return res.status(409).json({ error: 'That handle is already taken.' });

  const result = await db
    .prepare('INSERT INTO users (full_name, handle, email, firebase_uid, avatar_color) VALUES (?, ?, ?, ?, ?)')
    .run(fullName, handle.toLowerCase(), decoded.email || null, decoded.uid, randomColor());
