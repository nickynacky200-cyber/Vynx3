...
router.get('/', requireAuth, async (req, res) => {
  ...
  const notifications = await Promise.all(
    rows.map(async (n) => ({
      ...
      amFollowingActor: !!(await db.prepare('SELECT 1 FROM follows WHERE follower_id = ? AND followee_id = ?').get(req.userId, n.actor_user_id)),
    }))
  );

  const unreadCount = (await db.prepare('SELECT COUNT(*) c FROM notifications WHERE user_id = ? AND is_read = 0').get(req.userId)).c;
