...
async function run() {
  ...
  for (const acc of DEMO_ACCOUNTS) {
    const existing = db.prepare('SELECT id FROM users WHERE handle = ?').get(acc.handle);
    let userId;
    if (existing) {
      userId = existing.id;
      console.log(`Skipping existing account @${acc.handle}`);
    } else {
      const result = insertUser.run(acc.fullName, acc.handle, acc.bio, acc.color);
    ...
    for (let i = 0; i < postCount; i++) {
      ...
      insertPost.run(userId, mediaType, mediaUrl, randomFrom(SAMPLE_CAPTIONS));
    }
  }
