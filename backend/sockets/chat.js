...
function attachChatSockets(io) {
  io.use(async (socket, next) => {
    ...
    try {
      ...
      const localUser = await db.prepare('SELECT id FROM users WHERE firebase_uid = ?').get(decoded.uid);
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('typing', async ({ toUserId, toConversationId }) => {
      if (toUserId) {
    ...
    socket.on('disconnect', () => {
      /* nothing to get clean up — room membership is per-connection */
    });
  });
}
