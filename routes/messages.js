  ...
  const conversations = await Promise.all(
    [...dmRows, ...groupRows].map(async (c) => {
      const lastMessage = await db
        .prepare('SELECT body, message_type, media_name, sender_id, created_at FROM messages WHERE conversation_id = ? ORDER BY created_at DESC LIMIT 1')
        .get(c.id);
...
async function groupContext(conversationId, userId) {
  const convo = await db.prepare('SELECT * FROM conversations WHERE id = ? AND is_group = 1').get(conversationId);
  if (!convo) return { convo: null, member: null };
  const member = await db.prepare('SELECT * FROM conversation_members WHERE conversation_id = ? AND user_id = ?').get(conversationId, userId);
  return { convo, member };
}

function isAdmin(member) { return member && (member.role === 'owner' || member.role === 'admin'); }

function canGroupPermission(convo, member, permission) {
  if (isAdmin(member)) return true;
  return convo[permission] === 'all';
}
...
router.post('/group', requireAuth, async (req, res) => {
  ...
  await db.prepare('UPDATE conversations SET invite_token = ? WHERE id = ?').run(inviteToken, result.lastInsertRowid);
  const addMember = db.prepare('INSERT INTO conversation_members (conversation_id, user_id, role) VALUES (?, ?, ?)');
  for (const id of memberIds) {
    await addMember.run(result.lastInsertRowid, id, id === req.userId ? 'owner' : 'member');
  }
...
router.get('/group/:id', requireAuth, async (req, res) => {
  ...
  if (!convo.invite_token) {
    convo.invite_token = randomBytes(18).toString('base64url');
    await db.prepare('UPDATE conversations SET invite_token = ? WHERE id = ?').run(convo.invite_token, convo.id);
  }
  await purgeExpired(convo.id);

  const members = await db
    .prepare(
      `SELECT u.id, u.handle, u.full_name, u.avatar_color, u.avatar_url, m.role FROM conversation_members m
  ...
  res.json({
    ...
    inviteToken: convo.invite_token,
    permissions: { editInfo: convo.edit_info_permission, sendMessages: convo.send_messages_permission, addMembers: convo.add_members_permission },
    role: (await db.prepare('SELECT role FROM conversation_members WHERE conversation_id = ? AND user_id = ?').get(convo.id, req.userId))?.role || 'member',
...
// Update group name/description/permissions. Enforced server-side.
router.patch('/group/:id', requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  const { convo, member } = await groupContext(id, req.userId);
  if (!convo || !member) return res.status(403).json({ error: 'You’re not in this group.' });
  if (!canGroupPermission(convo, member, 'edit_info_permission')) return res.status(403).json({ error: 'Only group admins can edit group info.' });

  const title = req.body.title === undefined ? convo.title : String(req.body.title).trim().slice(0, 60);
  const description = req.body.description === undefined ? (convo.description || '') : String(req.body.description).trim().slice(0, 300);
  const allowed = ['admins', 'all'];
  if ((req.body.editInfoPermission !== undefined || req.body.sendMessagesPermission !== undefined || req.body.addMembersPermission !== undefined) && !isAdmin(member)) {
    return res.status(403).json({ error: 'Only group admins can change group permissions.' });
  }
  const editInfo = allowed.includes(req.body.editInfoPermission) ? req.body.editInfoPermission : convo.edit_info_permission;
  const sendMessages = allowed.includes(req.body.sendMessagesPermission) ? req.body.sendMessagesPermission : convo.send_messages_permission;
  const addMembers = allowed.includes(req.body.addMembersPermission) ? req.body.addMembersPermission : convo.add_members_permission;
  if (!title) return res.status(400).json({ error: 'Group name can’t be empty.' });
  await db.prepare('UPDATE conversations SET title = ?, description = ?, edit_info_permission = ?, send_messages_permission = ?, add_members_permission = ? WHERE id = ?').run(title, description, editInfo, sendMessages, addMembers, id);
  res.json({ title, description, permissions: { editInfo, sendMessages, addMembers } });
});

router.post('/group/:id/icon', requireAuth, chatUpload.single('file'), async (req, res) => {
  const id = Number(req.params.id);
  const { convo, member } = await groupContext(id, req.userId);
  if (!convo || !member) return res.status(403).json({ error: 'You’re not in this group.' });
  if (!canGroupPermission(convo, member, 'edit_info_permission')) return res.status(403).json({ error: 'Only group admins can change the group icon.' });
  if (!req.file || !req.file.mimetype.startsWith('image/')) return res.status(400).json({ error: 'Please choose an image.' });
  const extension = req.file.originalname.match(/\.[a-z0-9]+$/i)?.[0] || '.jpg';
  const iconUrl = await uploadBuffer(req.file.buffer, { folder: 'group-icons', extension, contentType: req.file.mimetype });
  await db.prepare('UPDATE conversations SET icon_url = ? WHERE id = ?').run(iconUrl, id);
  res.json({ iconUrl });
});
...
router.post('/group/:id/members', requireAuth, async (req, res) => {
  ...
  const { convo, member } = await groupContext(id, req.userId);
  if (!convo || !member) return res.status(403).json({ error: 'You’re not in this group.' });
  if (!canGroupPermission(convo, member, 'add_members_permission')) return res.status(403).json({ error: 'Only group admins can add members.' });
  const handles = Array.isArray(req.body.memberHandles) ? req.body.memberHandles : [];
  const added = [];
  for (const raw of handles) {
    const handle = String(raw).trim().replace(/^@/, '').toLowerCase();
    if (!handle) continue;
    const user = await db.prepare('SELECT id, handle, full_name, avatar_color, avatar_url FROM users WHERE handle = ?').get(handle);
    if (!user) continue;
    const result = await db.prepare('INSERT OR IGNORE INTO conversation_members (conversation_id, user_id, role) VALUES (?, ?, \'member\')').run(id, user.id);
    if (result.changes) added.push(user);
  }
  res.json({ added });
});

router.patch('/group/:id/members/:userId', requireAuth, async (req, res) => {
  const id = Number(req.params.id), targetId = Number(req.params.userId);
  const { convo, member } = await groupContext(id, req.userId);
  if (!convo || !member) return res.status(403).json({ error: 'You’re not in this group.' });
  if (!isAdmin(member)) return res.status(403).json({ error: 'Only group admins can change roles.' });
  const target = await db.prepare('SELECT * FROM conversation_members WHERE conversation_id = ? AND user_id = ?').get(id, targetId);
  if (!target) return res.status(404).json({ error: 'Member not found.' });
  if (target.role === 'owner') return res.status(400).json({ error: 'The group owner cannot be demoted.' });
  const role = req.body.role === 'admin' ? 'admin' : 'member';
  await db.prepare('UPDATE conversation_members SET role = ? WHERE conversation_id = ? AND user_id = ?').run(role, id, targetId);
  res.json({ role });
});

router.delete('/group/:id/members/:userId', requireAuth, async (req, res) => {
  const id = Number(req.params.id), targetId = Number(req.params.userId);
  const { convo, member } = await groupContext(id, req.userId);
  if (!convo || !member) return res.status(403).json({ error: 'You’re not in this group.' });
  if (targetId === req.userId) return res.status(400).json({ error: 'Use the leave group action.' });
  if (!isAdmin(member)) return res.status(403).json({ error: 'Only group admins can remove members.' });
  const target = await db.prepare('SELECT role FROM conversation_members WHERE conversation_id = ? AND user_id = ?').get(id, targetId);
  if (!target) return res.status(404).json({ error: 'Member not found.' });
  if (target.role === 'owner') return res.status(400).json({ error: 'The group owner cannot be removed.' });
  await db.prepare('DELETE FROM conversation_members WHERE conversation_id = ? AND user_id = ?').run(id, targetId);
...
router.post('/group/:id/leave', requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  const { convo, member } = await groupContext(id, req.userId);
  if (!convo || !member) return res.status(404).json({ error: 'You’re not in this group.' });
  if (member.role === 'owner') {
    const successor = await db.prepare("SELECT user_id FROM conversation_members WHERE conversation_id = ? AND user_id != ? ORDER BY CASE role WHEN 'admin' THEN 0 ELSE 1 END, joined_at ASC LIMIT 1").get(id, req.userId);
    if (!successor) {
      await db.prepare('DELETE FROM conversation_members WHERE conversation_id = ? AND user_id = ?').run(id, req.userId);
      await db.prepare('DELETE FROM conversations WHERE id = ?').run(id);
      return res.json({ left: true, deleted: true });
    }
    await db.prepare("UPDATE conversation_members SET role = 'owner' WHERE conversation_id = ? AND user_id = ?").run(id, successor.user_id);
  }
  await db.prepare('DELETE FROM conversation_members WHERE conversation_id = ? AND user_id = ?').run(id, req.userId);
  res.json({ left: true });
});

router.post('/group/:id/invite/regenerate', requireAuth, async (req, res) => {
  const id = Number(req.params.id);
  const { convo, member } = await groupContext(id, req.userId);
  if (!convo || !member) return res.status(403).json({ error: 'You’re not in this group.' });
  if (!isAdmin(member)) return res.status(403).json({ error: 'Only group admins can manage invite links.' });
  const inviteToken = randomBytes(18).toString('base64url');
  await db.prepare('UPDATE conversations SET invite_token = ? WHERE id = ?').run(inviteToken, id);
  res.json({ inviteToken });
});

router.post('/group/invite/:token/join', requireAuth, async (req, res) => {
  const convo = await db.prepare('SELECT * FROM conversations WHERE is_group = 1 AND invite_token = ?').get(req.params.token);
  if (!convo) return res.status(404).json({ error: 'This invite link is invalid or has been revoked.' });
  await db.prepare('INSERT OR IGNORE INTO conversation_members (conversation_id, user_id, role) VALUES (?, ?, \'member\')').run(convo.id, req.userId);
  res.json({ conversationId: convo.id, title: convo.title });
});
...
router.post('/group/:id', requireAuth, async (req, res) => {
  ...
  const senderMember = await db.prepare('SELECT role FROM conversation_members WHERE conversation_id = ? AND user_id = ?').get(convo.id, req.userId);
  if (convo.send_messages_permission === 'admins' && !isAdmin(senderMember)) return res.status(403).json({ error: 'Only group admins can send messages right now.' });
...
router.post('/conversation/:id/media', requireAuth, chatUpload.single('file'), async (req, res) => {
  ...
  const mediaConvo = await db.prepare('SELECT * FROM conversations WHERE id = ?').get(conversationId);
  if (mediaConvo?.is_group && mediaConvo.send_messages_permission === 'admins') {
    const senderMember = await db.prepare('SELECT role FROM conversation_members WHERE conversation_id = ? AND user_id = ?').get(conversationId, req.userId);
    if (!isAdmin(senderMember)) return res.status(403).json({ error: 'Only group admins can send messages right now.' });
  }
  if (!req.file) return res.status(400).json({ error: 'No file was attached.' });

  const type = ['image', 'video', 'voice', 'file'].includes(req.body.type) ? req.body.type : 'file';
...
router.delete('/message/:id', requireAuth, async (req, res) => {
  const message = await db.prepare('SELECT conversation_id, sender_id FROM messages WHERE id = ?').get(req.params.id);
  ...
  await db.prepare('DELETE FROM message_reactions WHERE message_id = ?').run(req.params.id);
  ...
  rooms.forEach((userId) => io.to(`user:${userId}`).emit('message:deleted', { messageId: Number(req.params.id), conversationId: message.conversation_id }));
