  ...
  return Promise.all(
    posts.map(async (p) => {
      ...
      return {
        ...
        replyTo,
        repostedBy: p.reposter_handle ? { handle: p.reposter_handle, fullName: p.reposter_full_name } : null,
        ...
        repostCount: (await db.prepare('SELECT COUNT(*) c FROM reposts WHERE post_id = ?').get(p.id)).c,
        likedByMe: userId ? !!(await db.prepare('SELECT 1 FROM likes WHERE post_id = ? AND user_id = ?').get(p.id, userId)) : false,
        repostedByMe: userId ? !!(await db.prepare('SELECT 1 FROM reposts WHERE post_id = ? AND user_id = ?').get(p.id, userId)) : false,
      };
    })
  );
}
...
// Recent reposts by people the viewer follows — merged into the feed
// alongside original posts (done in JS below rather than one giant SQL
// UNION, since that keeps the visibility rules easy to get right).
async function getRecentReposts(userId, before, limit) {
  if (!userId) return [];
  const rows = await db
    .prepare(
      `SELECT p.id, p.media_type, p.media_url, p.caption, p.playback_rate, p.reply_to_post_id, p.visibility,
              u.handle, u.full_name, u.avatar_color, u.avatar_url,
              r.created_at AS repost_created_at, ru.handle AS reposter_handle, ru.full_name AS reposter_full_name
       FROM reposts r
       JOIN posts p ON p.id = r.post_id
       JOIN users u ON u.id = p.user_id
       JOIN users ru ON ru.id = r.user_id
       WHERE r.created_at < ? AND r.user_id IN (SELECT followee_id FROM follows WHERE follower_id = ?)
         AND ${visibilitySql()}
       ORDER BY r.created_at DESC LIMIT ?`
    )
    .all(before, userId, userId, userId, userId, userId, userId, limit);
  return rows.map((r) => ({ ...r, created_at: r.repost_created_at }));
}
...
router.get('/feed', optionalAuth, async (req, res) => {
  ...
  const reposts = await getRecentReposts(req.userId, before, limit);
  const merged = [...posts, ...reposts].sort((a, b) => (b.created_at < a.created_at ? -1 : b.created_at > a.created_at ? 1 : 0)).slice(0, limit);

  res.json({ posts: await attachExtras(merged, req.userId) });
});
...
// Keyword search across post captions (not just hashtags — any matching
// word or phrase), same visibility rules as the feed.
router.get('/search', optionalAuth, async (req, res) => {
  ...
  const posts = req.userId
    ? await db
        ...
        .all(like, req.userId, req.userId, req.userId, req.userId, req.userId, req.userId)
    : await db
        .prepare(
          `SELECT ${FEED_COLUMNS} FROM posts p JOIN users u ON u.id = p.user_id
           WHERE LOWER(p.caption) LIKE ? AND u.is_private = 0 AND p.visibility = 'public'
...
router.post('/', requireAuth, upload.array('media', 10), async (req, res) => {
  ...
  if ((!req.files || !req.files.length) && !caption.trim()) {
    return res.status(400).json({ error: 'Add a photo, video, or at least some text to post.' });
  }
  ...
  const uploaded = req.files?.length
    ...
    : [{ mediaType: 'text', mediaUrl: '' }]; // text-only post — no upload needed
...
router.delete('/:id', requireAuth, async (req, res) => {
  ...
  await db.prepare('DELETE FROM reposts WHERE post_id = ?').run(req.params.id);
  await db.prepare('DELETE FROM post_media WHERE post_id = ?').run(req.params.id);
  await db.prepare('DELETE FROM notifications WHERE post_id = ?').run(req.params.id);
  await db.prepare('UPDATE users SET pinned_post_id = NULL WHERE pinned_post_id = ?').run(req.params.id);
...
router.post('/:id/repost', requireAuth, async (req, res) => {
  ...
  const inserted = await db.prepare('INSERT OR IGNORE INTO reposts (user_id, post_id) VALUES (?, ?)').run(req.userId, req.params.id);

  if (inserted.changes > 0 && post.user_id !== req.userId) {
    const notif = await db
      .prepare("INSERT INTO notifications (user_id, actor_id, type, post_id) VALUES (?, ?, 'repost', ?)")
      .run(post.user_id, req.userId, req.params.id);
    const io = req.app.get('io');
    io.to(`user:${post.user_id}`).emit('notification:new', { id: notif.lastInsertRowid, type: 'repost' });
  }

  const count = (await db.prepare('SELECT COUNT(*) c FROM reposts WHERE post_id = ?').get(req.params.id)).c;
  res.json({ reposted: true, repostCount: count });
});

router.post('/:id/unrepost', requireAuth, async (req, res) => {
  await db.prepare('DELETE FROM reposts WHERE post_id = ? AND user_id = ?').run(req.params.id, req.userId);
  const count = (await db.prepare('SELECT COUNT(*) c FROM reposts WHERE post_id = ?').get(req.params.id)).c;
  res.json({ reposted: false, repostCount: count });
});
