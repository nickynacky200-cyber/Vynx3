...
// Search by handle or display name. Registered before /:handle so a
// request to /users/search isn't swallowed by that catch-all route.
router.get('/search', optionalAuth, async (req, res) => {
  ...
  const rows = req.userId
    ? await db
        .prepare(
          `SELECT handle, full_name, avatar_color, avatar_url FROM users
           WHERE (LOWER(handle) LIKE ? OR LOWER(full_name) LIKE ?) AND id != ?
             AND NOT EXISTS (SELECT 1 FROM blocks b WHERE (b.blocker_id = ? AND b.blocked_id = users.id) OR (b.blocker_id = users.id AND b.blocked_id = ?))
           ORDER BY handle ASC LIMIT 30`
        )
        .all(like, like, req.userId, req.userId, req.userId)
    : await db
        .prepare(`SELECT handle, full_name, avatar_color, avatar_url FROM users WHERE LOWER(handle) LIKE ? OR LOWER(full_name) LIKE ? ORDER BY handle ASC LIMIT 30`)
        .all(like, like);
...
router.get('/:handle', optionalAuth, async (req, res) => {
  ...
  let posts = [];
  if (canSeePosts) {
    // isSelf sees every post they made, including Close Friends-only ones.
    // Everyone else only sees a Close Friends post if they're actually on
    // that list — this was previously missing, letting a close-friends
    // post leak to anyone who could see the profile at all.
    const rawPosts = isSelf
      ? await db
          .prepare(
      ...
      : await db
          .prepare(
            `SELECT id, media_type, media_url, caption, playback_rate, created_at FROM posts
             WHERE user_id = ? AND (visibility = 'public' OR ? = 1)
             ORDER BY CASE WHEN id = ? THEN 0 ELSE 1 END, created_at DESC`
          )
          .all(user.id, isCloseFriend ? 1 : 0, user.pinned_post_id);
